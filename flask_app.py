...
def load_league(league_name):
    ...
    # list_games and League both sort chronologically. Python's sort is stable,
    # so equal timestamps remain in database-id order too.
    for game, row in zip(league.games, rows):
        game.db_id = row["id"]

    return league


def _find_game(league, db_id):
    for game in league.games:
        if getattr(game, "db_id", None) == db_id:
            return game
    abort(404)
...
def edit_game(league_name, game_id):
    ...
    league = load_league(league_name)
    game = _find_game(league, game_id)
    return render_template(
        "edit_game.html",
        game=game,
        game_id=game_id,
...
def delete_game(league_name, game_id):
    try:
        storage.delete_game(game_id=game_id, league_name=league_name)
    except LookupError:
        abort(404)
    return redirect(url_for("index", league_name=league_name))
...
def player_profile(league_name, player_name):
    league = load_league(league_name)
    if player_name not in league.players:
        abort(404)
